...
//启动聊天窗口并连接聊天服务器
 function creatw() {
			...
			function initWebSocket() {
				if (window.WebSocket) {
					...
					websocket.onopen = function() {
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('_re_project_salt_'+user);
					}
			...
			win.on("close",function(){
				websocket.send('Leave_re_project_salt_');
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "_re_project_salt_no_"+toUser+"_re_project_salt_no_no_" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
