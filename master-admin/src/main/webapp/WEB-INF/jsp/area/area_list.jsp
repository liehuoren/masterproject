							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.AREA_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>${var.WAIT1}</td>
											<td class='center'>${var.WAIT2}</td>
											<td class='center'>${var.WAIT3}</td>
											<td class='center'>${var.WAIT4}</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>area/delete.do?AREA_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>area/goEdit.do?AREA_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
