										...
										<div style="float: left;">
											<!-- <table>
												<tr>
													...
													<td>&nbsp;地址：</td>
													<td><input type="text" name="dbAddress" id="dbAddress" value="123.56.234.98" maxlength="200" placeholder="数据库连接地址" title="数据库连接地址"/></td>
													<td>&nbsp;端口：</td>
												...
												<tr>
													...
													<td>&nbsp;密码：</td>
													<td><input type="password" value="njms_TEST_201610_pwd" name="password" id="password"  maxlength="32" placeholder="输入密码" title="密码"/></td>
													<td>&nbsp;库名：</td>
													<td><input type="text" value="masterproject_db" name="databaseName" id="databaseName"  maxlength="32" placeholder="输入数据库名" title="数据库名"/></td>
												</tr>
											</table> -->
										</div>
										<div style="float: left;margin-top:15px;margin-left: 20px;" id="db_connection_info">
...
<!-- 											</a> -->
	...
	<script type="text/javascript">
		...
		function connDb(){
			 top.jzts();
			 $("#db_connection_info").html("<p>正在连接到数据库。。。</p>");
			 $.ajax({
					type: "POST",
...
// 			    	data: {dbtype:dbtype,dbAddress:dbAddress,dbport:dbport,username:username,password:password,databaseName:databaseName},
					...
					success: function(data){
						 $.each(data.list, function(i, list){
							 if("ok" == list.msg){
								 $("#db_connection_info").html("<p>成功连接到数据库！</p>");
								 $("#fhdb").tips({
										side:1,
							            msg:'连接成功',
							            bg:'#009933',
								 ...
								 $("#db_connection_info").html("<p>连接失败,检查配置文件参数是否正确！</p>");
								 $("#fhdb").tips({
										side:3,
							            msg:'连接失败,检查配置文件参数是否正确！',
							            bg:'#cc0033',
							            time:15
							        });
							 }
		...
		function productCode(table){
			...
			databaseName = databaseName.replace("#","%23"); */
			 ...
			 diag.Drag=true;
			 diag.Title ="代码生成器";
			 diag.URL = '<%=basePath%>recreateCode/goProductCode.do?table='+table;
			 diag.Width = 800;
			 diag.Height = 500;
