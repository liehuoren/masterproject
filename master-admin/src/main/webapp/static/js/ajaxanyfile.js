...
try {
	myAnyFile = {
			...
			ajaxFile:function(fileId,showImg,imgValue,input,params){
			    ...
			    var FileController = my.getBasePath()+"/pictures/uploadfile.do?"+new Date().getTime(); // 接收上传文件的后台地址   
				...
				if(my.request.status=='200'){
					...
					if(e.currentTarget.response){
						...
						if(res.code=='000000'){
							...
							$('#'+my.showImg).attr("href",res.data.showImg).show();
							$("#_the_line_text").text("上传成功!");
							$("#"+my.fileId).tips({
								side:2,
					            msg:'上传成功!',
					            bg:'#AE81FF',
