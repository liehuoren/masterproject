...
public class Uploader {
	 ...
	 * 文件上传到指定的目录
	 */
	private String UPLOAD_FOLDER_PATH = DbFH.getPprVueBypath("upload.properties").getProperty("upload.imagePath");
	/**
	 * 图片访问前缀
	 */
	private String UPLOAD_REQUESTPATH = DbFH.getPprVueBypath("upload.properties").getProperty("upload.requestPath");
	...
	//原上传方法
	public void upload_bak() throws Exception {
	...
	public void upload() throws Exception {
		...
		try {
			String savePath = this.getFolder_new("");//目录由配置文件提供
			...
			this.fileName = this.getName(this.originalName);
			this.type = this.getFileExt(this.fileName);
			this.url = savePath + "/" + "UE" +this.fileName;
			
			FileOutputStream fos = new FileOutputStream(
					UPLOAD_FOLDER_PATH+this.url);
			BufferedInputStream bis = new BufferedInputStream(this.inputStream);
			byte[] buff = new byte[128];
			int count = -1;
	 ...
	 * @return
	 */
	private String getFolder_new(String path) {
		...
		path += uploadPath;
		File dir = new File(this.UPLOAD_FOLDER_PATH+path);
		if (!dir.exists()) {
			try {
				dir.mkdirs();
			} catch (Exception e) {
	...
	public String getUrl() {
		return this.UPLOAD_REQUESTPATH+this.url;
	}
