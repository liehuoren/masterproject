package com.master.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("_re_project_salt_")){
			this.userjoin(message.replaceFirst("_re_project_salt_", ""),conn);
		}if(null != message && message.startsWith("Leave_re_project_salt_")){
			this.userLeave(conn);
		}if(null != message && message.contains("_re_project_salt_no_")){
			String toUser = message.substring(message.indexOf("_re_project_salt_no_")+10, message.indexOf("_re_project_salt_no_no_"));
			message = message.substring(0, message.indexOf("_re_project_salt_no_")) +"[私信]  "+ message.substring(message.indexOf("_re_project_salt_no_no_")+12, message.length());
